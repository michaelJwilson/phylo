...
[Unreleased]
Added
Jukes-Cantor Simulator (#55): Implemented a k-state (k=4) sequence simulator in phylo.sim, generating alignments and ancestor trees (Newick) from a typed simulation_params.yaml; validated against the closed-form JC transition probabilities within a stated Monte Carlo tolerance across several site/taxa sizes. Promotes numpy and pyyaml to core dependencies.
...
Removed
Deprecated automated changelog fragments (changelog.d/) and associated infra/changelog.py script in favor of a standard flat file.
