...
\paragraph{Intended reader.} This document is written for a well-educated
developer with baseline scientific and performance-computing background, but
no expert knowledge of phylogenetics. The body stays streamlined --- citing
sources rather than re-deriving standard material inline --- and pushes
required background that is not phylo-specific into
Appendix~\ref{app:background}, referenced from the point of use. It is a
best-effort high-level overview of the current best-known approach
(simulation, models), not an exhaustive record; supporting detail, plots,
and results live in the documents it links to.
