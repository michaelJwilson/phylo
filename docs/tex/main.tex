...
The graph this section states abstractly is a \texttt{phylo.sim.graph.PottsGraph} in code: nodes, one $J_{ij}$ per edge, and an explicit boundary condition. \texttt{lattice\_graph} builds an $N$-dimensional lattice as an instance of it -- a 1-D chain is $N=1$, not a second implementation -- and \texttt{phylo.sim.potts.simulate\_potts} draws configurations from the result, exactly by transfer matrix on an open chain and by single-site Gibbs/heat-bath MCMC otherwise.
