...
document may say. `nb/` is the exception that proves it -- CI does not
re-execute a notebook, so its own rules below say what stands in for that.
...
- **A notebook under `nb/` is not regenerated by CI**, so what keeps it true
  is the pull request that changes what it prints. Nothing there may state a
  result the regression suite does not also pin, and the **Further Work**
  section at the end of each names, with its issue number, what the notebook
  could not demonstrate because the feature is not built. That section is
  load-bearing: a notebook that quietly omitted the unbuilt half would read as
  a complete tour of an incomplete repository. Adding Jupyter and a job that
  re-executes them would put `nb/` under the same contract as `tex/`, and
  needs the dependency sign-off root `CLAUDE.md` requires.
