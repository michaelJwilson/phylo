...
# science -- it orchestrates phylo.qa and latexmk, and knows nothing about
# topologies, models, or which fixture renders which figure (see this
# directory's CLAUDE.md, and phylo.qa.manifest for the figures themselves).
set -euo pipefail
...
# check relies on.
# One home for the constant: phylo.qa.build pins it for the figures, and this
# reads it back so latexmk stamps the PDF with the same clock.
SOURCE_DATE_EPOCH="$(uv run python -c \
  'from phylo.qa.build import SOURCE_DATE_EPOCH; print(SOURCE_DATE_EPOCH)')"
...
# Which figures exist and what renders each one is `phylo.qa.manifest`, not a
...
uv run python -m phylo.qa.build \
