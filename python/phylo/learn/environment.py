...
This module is to ``phylo.learn`` what ``objective.py`` is to ``phylo.opt``,
...
this one is written against no application at all: ``phylo.learn`` imports
nothing from ``phylo.sim``, ``phylo.likelihood`` or ``phylo.search``, asserted
...
state under *known* parameters, never a quantity solved for by ``phylo.opt``.
...
class Environment[S, A](Protocol):
    ...
    :func:`rollout` and the exact enumeration in :mod:`phylo.learn.exact`
    ...
    def features(self, state: S, actions: Sequence[A]) -> torch.Tensor:
        ...
        **unidentifiable**: the policy is a softmax over these scores, and a
        ...
        ``phylo.opt.constrain.log_simplex`` fixes, and the reason no
