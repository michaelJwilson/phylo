...
``phylo.search.infer``, which counts a budget in candidate fits for the same
...
from phylo.learn.policy import Policy
...
def greedy_rollout[S, A](
    ...
    ``phylo.search.infer`` applies to topologies, on a landscape small enough
