...
from phylo.qa.manifest import FIGURES, FigureSpec, cited_stems, select, unknown_stems
...
def selected(
    ...
    if only:
        ...
        if missing:
            msg = f"no phylo.qa.manifest entry renders {sorted(missing)}"
    ...
    if missing:
        msg = (
            f"{main_tex} cites {sorted(missing)}, which no phylo.qa.manifest "
