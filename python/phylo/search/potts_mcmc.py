...
class PottsMove(StrEnum):
    ...
    A ``StrEnum`` for the reason `phylo.search.infer.MoveSet` is one: an
...
def energies(graph: PottsGraph, field: np.ndarray, states: np.ndarray) -> np.ndarray:
    ...
    Taken from :func:`phylo.likelihood.potts.log_weights` rather than written
...
def _single_site_sweep(
    ...
    update `phylo.sim.potts._simulate_gibbs` uses --- restated here for a
