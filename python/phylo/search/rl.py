...
This is the application instance of :class:`phylo.learn.environment.Environment`,
and it lives here rather than in ``phylo.learn`` for the reason the
phylogenetic ``Objective`` lives in ``phylo.likelihood``: ``learn/`` may
...
:mod:`phylo.qa.rl_reward_surface` measures how far apart the two are rather
...
class TopologyEnvironment:
    ...
    branch_length : float
        ...
        :mod:`phylo.qa.rl_reward_surface` measures how much the resulting
    ...
    def evaluations(self) -> int:
        ...
        ``phylo.search.infer`` uses and the reason both are stated in
    ...
    def features(self, state: Topology, actions: Sequence[Topology]) -> torch.Tensor:
        ...
        :mod:`phylo.qa.rl_reward_surface` does.
        """
