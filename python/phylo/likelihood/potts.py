...
Belief propagation (:mod:`phylo.likelihood.belief_propagation`) is checked
against the first where it is exact and *measured* against the second where it
is not.

The model is the one :func:`phylo.sim.potts.simulate_potts` samples from, and
...
def enumerate_potts(
    ...
    :mod:`phylo.likelihood.belief_propagation` -- only the model convention,
...
def strip_log_partition(
    ...
    shape : tuple[int, int]
        ...
        node indexing :func:`phylo.sim.graph.lattice_graph` uses.
    boundary : BoundaryCondition
        ``OPEN`` only.
    coupling : float
        Uniform ``J``.
