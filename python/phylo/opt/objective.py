...
mentions no model, and ``phylo.opt`` imports nothing from ``phylo.sim``,
``phylo.likelihood`` or ``phylo.search`` -- asserted by a test, not left to
review.
...
class Objective(Protocol):
    ...
    def __call__(self, theta: torch.Tensor) -> torch.Tensor:
        ...
        Minimization is the convention throughout ``phylo.opt``, so a
