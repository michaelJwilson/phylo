...
def test_the_new_door_is_the_old_one(objective: Objective, theta: torch.Tensor) -> None:
    ...
    # a match can be.
    from snakes_and_ladders.opt.fit import constrained_standard_errors
...
def test_a_multi_start_interval_belongs_beside_the_spread_that_qualifies_it() -> None:
    ...
    result = fit_from(
        objective, RandomRestart(3, scale=0.2, rng=np.random.default_rng(4))
    )

    errors = standard_errors_at(objective, objective.constrain(result.best.theta))
...
def test_the_delta_method_interval_and_the_sampled_posterior_agree() -> None:
    # The comparison `hmc.py`'s docstring promises. A version of it already
    # existed -- a raw Hessian in *unconstrained* coordinates against grid
    # quadrature, at rtol=0.15. This is the missing half: the delta-method
    # interval on the parameters a person names, against a chain.
    #
