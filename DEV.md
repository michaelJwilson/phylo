...
| `python/phylo/opt/` | Model-agnostic continuous parameter fitting via autodiff (PyTorch): the `Objective` interface, shared constraint maps, and the Potts and HMM reference instances. Imports nothing from `likelihood/`, `search/`, or a phylogenetics-specific submodule of `sim/`, asserted by test; the model-agnostic `sim/graph.py` and `sim/potts.py` are the one exception (`opt/CLAUDE.md`). |
