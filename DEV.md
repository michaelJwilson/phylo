...
| `python/phylo/search/` | Move sets, temperature schedules, and the hill-climbing search (`infer.py`) that joins them to `opt/`. The phylogenetic RL environment belongs here too, for the reason the phylogenetic `Objective` belongs in `likelihood/`: `learn/` may import no application module. |
