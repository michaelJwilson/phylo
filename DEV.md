...
* **A PR implements a plan already approved.** The ticket carries a plan comment before any code exists, the issue is labelled `planned`, and a maintainer applies `approved`; only then may the pull request open, and it implements that plan. Opening it labels the ticket `drafted`, which is the lifecycle's last step before the merge closes it — the label distinguishes a ticket still waiting on a reviewer from one whose work is written and waiting on CI. A plan that turns out to be flawed gets a revised plan posted to the thread, not a silent correction in the diff.
