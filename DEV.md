...
* **Once a branch is created for an approved plan**, the first thing posted is a single issue comment naming the branch (and, once opened, the PR number) before any further commit is pushed. A session that fails, is interrupted, or is deferred mid-implementation then leaves a ticket that already points at the in-flight branch, instead of one requiring a search across open branches and PRs to find it.
